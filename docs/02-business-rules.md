...
- **İstisna — kritik tekil yorum sinyali:** own tarafında bir temaya değinen yorumlardan **en az biri** sağlık/güvenlik zararı, ciddi bir etik/yasal risk ya da dolandırıcılık iddiası içeriyorsa (Aşama 1 modeli bu temayı `theme_summary.severity = 'critical'` olarak işaretler — bkz. `05-ai-pipeline.md`, `06-prompts.md`), yukarıdaki **5 mention eşiği atlanır**: tek bir olumsuz yorum bile `absolute_quality` görevi üretebilir. Gerekçe: ciddiyet sıklıktan bağımsızdır — "diş kaybına neden olan tedavi" gibi tek bir yorum, 5 tane "bekleme süresi uzun" yorumundan daha kritiktir; sadece frekans ölçen bir eşik bunu hep susturur. `critical` işaretli temalar impact score hesabında da (own_deficiency bileşeni) etraftaki olumlu yorumlarla sulandırılmaz, tam eksiklik (100) kabul edilir (`09-task-engine.md`).
