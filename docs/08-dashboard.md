...
**"Uyarılar" listesi (Faz 2.7).** `delta.alerts` doluysa (bkz. `02-business-rules.md` Bölüm G, `src/lib/analysis/competitor-alerts.ts`) tema chip'lerinin altında ayrı bir `Badge` listesi (`AnalysisDeltaAlerts`, `analysis-delta-alerts.tsx`) — her rozet bir rakip uyarısını (yeni yorum patlaması / puan değişimi / negatif tema patlaması) özetler. `alerts` boş/undefined ise (bu döngüde uyarı yok ya da eski bir `delta` satırı) liste hiç render edilmez.
...
**Tasks** — Tüm görevler, `status`/`priority` filtreli liste. Her görev: başlık, açıklama, impact/effort göstergesi, hangi rakip(ler)den doğduğu, tamamla/reddet aksiyonları. Görev kartlarında ayrıca kod tarafında hesaplanan bir kanıt satırı gösterilir — `theme_summary`'den own vs rakip mention sayısı kıyası (`competitive_gap` için rakip pozitif/own pozitif, `absolute_quality` için own negatif). Görevin teması ile `theme_summary` satırları eşleşmezse (AI'ın ürettiği tema adı drift ederse) satır gösterilmez, uydurma sayı verilmez. Kanıt satırının hemen altında, mevcutsa, bir **sonuç takibi satırı** gösterilir: "Oluşturulduğunda olumsuz bahsedilme %45 → şimdi %20" gibi baseline→latest kıyası + iyileşti/kötüleşti/değişmedi rozeti (bkz. `09-task-engine.md` "Görev sonuç takibi"). Baseline ya da latest ölçümü yoksa (ör. henüz ikinci bir analiz döngüsü geçmediyse) satır hiç gösterilmez.

**Tasks → History (Faz 1.1)** — `/business/tasks/history` alt-görünümü: `status in ('done','dismissed')` görevlerin salt-okunur listesi. Her satırda status rozeti (Tamamlandı/Reddedildi) ve `done` görevler için `completed_at` tarihi (`dismissed`'in kendi timestamp'i yok, tarih gösterilmez; sıralamada `created_at`'a düşülür). Tasks sayfasıyla arasında karşılıklı link vardır, ayrı bir üst sekme değildir. Aynı kart bileşeni (`TaskCardBody`) kullanıldığı için sonuç takibi satırı burada da görünür — hatta ürünün "işe yaradı mı?" kanıtının en anlamlı göründüğü yer burasıdır: bir görev `done` işaretlendikten sonraki analiz döngülerinde tema gerçekten iyileşti mi?

**Competitors** — Seçilen rakiplerin kartları: puan, yorum sayısı, güçlü/zayıf temalar özeti. Rakip ekleme/çıkarma buradan yönetilir (limit: `02-business-rules.md` Bölüm A). **Canlı puan (Faz 2.7):** resmi puanın hemen altında, doluysa (`competitors.recent_rating IS NOT NULL`) "Son {window} gün: {rating} ★ ({n} yorum)" satırı — `null` ise (henüz `RECENT_RATING_MIN_REVIEWS`e ulaşmadıysa) satır hiç gösterilmez, uydurma/eksik veri yok (bkz. `02-business-rules.md` Bölüm F, `competitor-recent-rating-line.tsx`).
...
**Trend** — `clinic_score_history`'den zaman serisi grafiği (Clinic Score ve Competitor Rank'in zaman içindeki değişimi). Veri biriktikçe anlamlı olur — ilk haftalarda "yeterli veri birikiyor" mesajı gösterilir. **Canlı puan serisi (Faz 2.7):** aynı grafikte, `clinic_score_history.recent_ratings` jsonb'sinden türetilen iki ek çizgi — own `recent_rating` ve o anki tüm rakiplerin `recent_rating` **medyanı** (tek bir rakip değil — bkz. `02-business-rules.md` Bölüm F, `src/lib/analysis/recent-ratings.ts` `median`/`extractRecentRatingTrendPoint`). Bir snapshot'ta ilgili değer null ise (henüz eşiğe ulaşılmadıysa) o nokta atlanır (`connectNulls`), çizgi kesilmez.
